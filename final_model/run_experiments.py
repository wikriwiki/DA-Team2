...
def grid_search():
    ...
    combinations = list(itertools.product(lrs, hidden_dims, dropouts, weight_decays))
    
    for i, (lr, hidden, dropout, wd) in enumerate(combinations):
        exp_name = f"grid_{i}_lr{lr}_h{hidden}_d{dropout}_wd{wd}"
        cmd = (
            f'"{PYTHON_EXEC}" {TRAIN_SCRIPT} '
            f'--lr {lr} --hidden_dim {hidden} --dropout {dropout} --weight_decay {wd} '
